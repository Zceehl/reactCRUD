...
const EditIngredientScreen = () => {
    ...
    useEffect(() => {
        if (ingredient) {
            ...
            setIngredientData({
                ingredient_name: parsedIngredient.ingredient_name,
                unit: parsedIngredient.unit,
                stock_quantity: parsedIngredient.stock_quantity.toString(),
                unit_cost: parsedIngredient.unit_cost.toString(),
                minimum_stock: parsedIngredient.minimum_stock.toString()
            });
        }
    }, [ingredient]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            const parsedIngredient = JSON.parse(ingredient);
            const result = await InventoryService.updateIngredient(parsedIngredient.$id, ingredientData);
            if (result.success) {
                Alert.alert('Success', 'Ingredient updated successfully', [
                    { text: 'OK', onPress: () => router.back() }
                ]);
            } else {
                Alert.alert('Error', result.error || 'Failed to update ingredient');
            }
        } catch (error) {
